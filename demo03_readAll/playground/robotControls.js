...
export function setupControlPanel() {
  ...
  // 舵机角度控制
  const applyAnglesButton = document.getElementById('applyAngles');
  if (applyAnglesButton) {
    applyAnglesButton.addEventListener('click', () => {
      applyServoAngles();
    });
  }
}
 ...
 * 应用舵机角度设置
 */
function applyServoAngles() {
  // 读取所有舵机的角度输入
  const angles = {
    1: parseFloat(document.getElementById('servo1Angle').value) || 0,
    2: parseFloat(document.getElementById('servo2Angle').value) || 0,
    3: parseFloat(document.getElementById('servo3Angle').value) || 0,
    4: parseFloat(document.getElementById('servo4Angle').value) || 0,
    5: parseFloat(document.getElementById('servo5Angle').value) || 0,
    6: parseFloat(document.getElementById('servo6Angle').value) || 0
  };
  
  // 验证角度范围
  for (let i = 1; i <= 6; i++) {
    if (angles[i] < -90 || angles[i] > 90) {
      showAlert('joint', `舵机 ${i} 角度超出范围 (-90 到 90 度)`, 3000);
      return;
    }
  }
  ...
  if (window.robot && window.robot.joints) {
    // 遍历所有关节
    Object.values(window.robot.joints).forEach((joint, index) => {
      // 只处理前6个关节（对应6个舵机）
      if (index < 6) {
        const servoId = index + 1;
        const angleRad = angles[servoId] * Math.PI / 180; // 转换为弧度
        
        // 检查关节是否在限制范围内
        if (isJointWithinLimits(joint, angleRad)) {
          joint.setJointValue(angleRad);
        } else {
          showAlert('joint', `关节 ${index + 1} 角度超出限制范围`, 3000);
        }
      }
    });
  }
  
  // 如果连接了真实舵机，也更新真实舵机的位置
  if (isConnectedToRealRobot) {
    for (let i = 1; i <= 6; i++) {
      // 将角度转换为舵机位置值（-90到90度对应0到1023）
      const position = Math.round((angles[i] + 90) * (1023 / 180));
      writeServoPosition(i, position);
    }
  }
}